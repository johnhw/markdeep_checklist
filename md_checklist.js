        ...
        if (element.classList.contains("target") && name.startsWith("toc")) {
            ...
            while (p_element != null) {
                ...
                if (p_element.tagName && p_element.tagName.startsWith("H") && !attached) {
                    var toc_name = "'" + toc + "'";
                    p_element.innerHTML = '<button onclick="reset_checklist(' + toc_name + ')" class="clear-button"> &#x274c; </button> ' + p_element.innerHTML;
                    attached = true;
                }
...
function make_checks() {
    ...
    if (checklist_elements === null || checklist_elements === undefined) {
        checklist_elements = {};
    }
